...
atom 
  ...
  where 
    pNumber' 
      =  mkInt <*> (read <$> many1 digit <* notFollowedBy (digit <|> char '.'))
...
instance (SingI n', SingI n, (n' > n) ~ True) => EPrec ctx n' PU.< EPrec ctx n where 
  upcast = case sing @n of 
    SNat -> case 
      ...
      (Just Refl,_,_) -> OfHigher0
      (_,Just Refl,_) -> OfHigher1
      (_,_,Just Refl) -> OfHigherPostfixPrec
  ...
  downcast t 
    ...
    $ case 
      ...
      (Just Refl,_,_,_) -> case t of
        OfHigher0 f -> genericDowncast f 
      (_,Just Refl,_,_) -> case t of 
        OfHigher1 f -> genericDowncast f
        _           -> Nothing
      (_,_,Just Refl,_) -> case t of 
        OfHigherPostfixPrec f -> genericDowncast f
        _           -> Nothing
      (_,_,_,Just Refl) -> case t of 
        _           -> Nothing
...
yieldVarName x = case sing @n of 
    SNat -> case 
      ...
      (Just Refl,_,_,_) -> case x of 
        OfHigher0 x' -> yieldVarName x'
      (_,Just Refl,_,_) -> case x of 
        OfHigher1 x' -> yieldVarName x'
        _            -> Nothing 
      (_,_,Just Refl,_) -> case x of 
        OfHigherPostfixPrec x' -> yieldVarName x'
        _            -> Nothing 
      (_,_,_,Just Refl) -> case x of 
        PVar _ s -> Just s 
        _        -> Nothing 
      _                         -> error "Error. Upcast Expression Precedences must be one of: 0,1,4,6,7,8,Postfix,Prefix."
